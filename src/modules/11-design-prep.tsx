...
import { X, Copy, CheckCheck, ExternalLink, Languages } from "lucide-react";
...
const designPromptEn = `Use the attached DESIGN.md as the design reference for this project.
Apply its colors, typography, spacing, and card/button styles naturally to the current app.
Keep all existing features, content, and screen structure, and keep every visible text in Korean.
Do not copy the original site's logo, brand name, or images.`;
...
const fontPromptEn = `Apply the attached font file to this project.
Font name: [downloaded font name]
Use this font only for headings and key sections, and keep an easy-to-read Korean font for body text.
If the font does not support Korean, use it only for Latin text and keep the default font for Korean.
Do not change the existing layout or features, and make sure it looks good on mobile.`;
...
function CopyBlock({ code, codeEn, doneMsg }: { code: string; codeEn?: string; doneMsg: string }) {
  const [copied, setCopied] = useState(false);
  const [msg, setMsg] = useState("");
  const [lang, setLang] = useState<"ko" | "en">("ko");
  const shown = lang === "en" && codeEn ? codeEn : code;
  const onCopy = async () => {
    try {
      ...
      <div className="absolute top-2 right-2 flex items-center gap-1.5">
        {codeEn && (
          <button
            ...
            className="inline-flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-md bg-white/10 hover:bg-white/20 text-white/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-coral"
          >
        ...
        >
          ...
          <div className="overflow-y-auto p-5 sm:p-6 space-y-5">
            ...
            <div className="bg-surface-cream-strong rounded-lg p-4 sm:p-5">
              ...
              <CopyBlock code={designPrompt} codeEn={designPromptEn} doneMsg="디자인 적용 프롬프트가 복사되었습니다." />
    ...
    <Section title="만들기 전, 디자인 준비하기">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="bg-surface-card rounded-2xl p-6 flex flex-col">
          ...
          <CopyBlock code={fontPrompt} codeEn={fontPromptEn} doneMsg="폰트 적용 프롬프트가 복사되었습니다." />
